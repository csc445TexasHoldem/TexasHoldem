...
public class GameServerThread extends Thread implements TexasHoldemConstants{
   ...
   int DROP_TIMER = TexasHoldemConstants.HEARTBEAT_INTERVAL * 10;
   byte[] packSize = new byte[TexasHoldemConstants.MAX_PACKET_SIZE];
   byte[] info;
   ...
   public void run() {
      ...
      while (gameSession /* playing the game until 
             only one player remains, then maybe terminate,
             or we could bounce back up to lobby to recruit more
             players */) {
         try {
            socket.receive(packet);
            info = SharedUtilities.toByteArray(packet.getData());
